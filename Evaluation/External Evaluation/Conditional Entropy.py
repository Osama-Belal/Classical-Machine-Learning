...
def calculate_entropy(labels_true, labels_pred):
    ...
    for cluster in pred_cluster_indices.values():
        ...
        cluster_entropy = 0
        for count in true_cluster_counts:
            if count != 0:
                cluster_entropy += - (count / len(cluster)) * np.log2(count / len(cluster))
        entropy += cluster_entropy * len(cluster) / len(labels_true)
...
for C in K:
    ...
    print("K = ", C, "clusters ------------------------------------------------------")
    print("Entropy of 3_NN_similarity: ", calculate_entropy(ground_truth, cl1))
    print("Entropy of K-means: ", calculate_entropy(ground_truth, cl2))