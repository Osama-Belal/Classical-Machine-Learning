...
def calculate_purity(labels_true, labels_pred):
    ...
    # Iterate over predicted clusters
    for cluster in pred_cluster_indices.values():
        # Retrieve true cluster labels for the indices in the predicted cluster
        true_cluster_counts = np.bincount([labels_true[i] for i in cluster])
        # Add the maximum count to purity
        purity += np.max(true_cluster_counts)
...
for C in K:
    ...
    print("Purity of 3_NN_similarity: ", calculate_purity(ground_truth, cl1))
    print("Purity of K-means: ", calculate_purity(ground_truth, cl2))
